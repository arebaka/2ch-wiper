...
class Post:
	def __init__(self, schema, mode, triggerForm):
		self.ID = str(schema["num"])
		self.comment = self.set_comment(schema["comment"], mode, triggerForm)
		self.sage = self.set_sage(schema)
		self.num = schema["number"]
		self.medias = []
...
class Thread:
	def __init__(self, board, ID, mode, triggerForm):
		...
		self.schema = json.loads(requests.get(''.join(["https://2ch.hk/", board, "/res/", ID, ".json"])).text)
		self.postsCount = self.schema["posts_count"] + 1
		self.lastID = str(self.schema["max_num"])
		self.posts = self.download_posts(mode, triggerForm)
		self.loaf = ""
...
class Setup:
	def __init__(self, args):
		...
		self.cpFile, self.bansFile, self.fullFile = self.set_encoding()
		self.board = args[1]
		self.thread = args[2]
		self.chaos = args[3]
		self.potocksCount = int(args[4])
		self.TIMEOUT, self.PAUSE = self.set_consts(self.potocksCount)
		self.solver, self.key, self.keyreq = self.set_key(int(args[6]), args[7])
		self.proxyRepeatsCount = int(args[8])
		self.mode, self.pastes, self.bigPaste = self.set_mode(int(args[9]))
		if self.mode == 8:
			self.minBan = int(args[10])
			self.maxBan = int(args[11])
		self.catalog = 0
		self.threads = []
		if self.thread != "0":
			self.triggerForm, self.shrapnelCharge, self.targetThread = self.set_trigger(int(args[12]), int(args[13]), int(args[14]), args)
		else:
			self.triggerForm = 0
			self.shrapnelCharge = 0
		self.mediaKind, self.mediaPaths, self.mediasCount = self.set_media(int(args[15]), args[16], int(args[17]))
		self.sageMode = int(args[18])
		self.shakalPower = int(args[19])
		if args[20] == "1": self.shakalColor = True
		else: self.shakalColor = False
		if args[21] == "1": self.shakalAffine = True
		else: self.shakalAffine = False
		if args[22] == "1": self.toPNG = True
		...
		try:
			while counter < self.setup.proxyRepeatsCount:
				...
				if (post.prepare(self.setup.TIMEOUT)):
					...
					if self.thread != "0":
						black_anus = random.randint(1, len(thread.posts)-1)
					...
					if self.setup.mode == 1:
						...
						post.set_text(trigger + "https://2ch.hk" + "/b/res/MZx7W05J2ch.hk" * 500)
					...
					if self.setup.mediaKind > 0:
						try:
							...
							for mediaNum in range(mediasCount):
								if self.setup.mediaKind != 3:
									blue_anus = random.randint(0, len(self.setup.mediaPaths)-1)
